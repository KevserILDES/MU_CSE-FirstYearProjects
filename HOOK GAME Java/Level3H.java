...
//This class is for the hard part of the third level of the game
public class Level3H extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Timeline animationTime = new Timeline(new KeyFrame(Duration.seconds(STARTTIME+1),e->{
			if(timeSeconds.get()==0 ){
				...
				try {
					lev3h.start(primaryStage);//call the start method
				} catch (Exception e1) {
		...
		circleC.setOnMouseClicked(e -> {//when player click circle C
			if((vBLineC.getEndY()-vBLineC.getStartY())==20){//prevent to click circle more times
				...
				Timeline animationThinC=new Timeline(new KeyFrame(Duration.millis(50),el -> {
					...
					if(vBLineC.getStrokeWidth()==0.50 && timeSeconds.get()!=0){
						animationTime.pause();
