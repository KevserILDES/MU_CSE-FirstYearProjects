...
//This class is for the hard part of the second level of the game
public class Level2H extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		// TODO Auto-generated method stub
		Pane pane=new Pane();//creating new pane
		...
		Integer STARTTIME = 12;//starting second
		...
		Timeline animationTime = new Timeline(new KeyFrame(Duration.seconds(STARTTIME+1),e->{
			if(timeSeconds.get()==0 ){
				...
				try {
					lev2h.start(primaryStage);//call the start method
				} catch (Exception e1) {
		...
		pane.getChildren().addAll(time,textTime);//adding all nodes
		...
		circleD.setOnMouseClicked(e -> {
			if((hBLineD.getEndX()-hBLineD.getStartX())==20){//prevent to click circle more times
				...
				Timeline animationThinD=new Timeline(new KeyFrame(Duration.millis(50),el -> {
					...
					if(hBLineD.getStrokeWidth()==0.50 && timeSeconds.get()!=0){
						animationTime.pause();
						pane.getChildren().removeAll(hBLineD,vLLineD,circleD);
						Level3H lev3h=new Level3H();
						try {
							lev3h.start(primaryStage);
						} catch (Exception e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
